...
1) Euler #7: получить 10 001-е простое число, показать разные приёмы ФП: хвостовая и обычная рекурсия, цепочка filter/fold, map, императивный цикл и ленивые последовательности.
2) Euler #24: найти миллионную лексикографическую перестановку цифр `0..9`, показать факториальную позицию, алгоритм next-permutation и ленивые последовательности.
...
- Решения задачи (модуль `Fp_first_lab.Euler7`):
  ...
  - `modular_pipeline`: поток `ints_from 2 |> Seq.filter is_prime`, n-й элемент берётся через `Seq.drop` + `Seq.uncons`.
  - `mapped_generation`: генерация пар через `map`, фильтрация по признаку простоты и выбор нужного элемента потока.
  - `loop_based`: императивный `while`-цикл для сравнения с традиционным стилем.
  - `lazy_seq_based`: работа с бесконечным ленивым списком простых (`primes_seq ()`, выбор n-го элемента через `Seq.drop` + `Seq.uncons`).
- Решения задачи (модуль `Fp_first_lab.Euler24`) — миллионная лексикографическая перестановка цифр `0..9`:
  ...
  - `lazy_seq_based`: ленивый `Seq` из перестановок и выбор нужного элемента через `Seq.drop` + `Seq.uncons`.
...
- Альтернативная проверка на Python: `python3 python/euler.py -p 7 -n 10001` или `python3 python/euler.py -p 24 -n 1000000`.
...
- Наивная рекурсия быстро уходит в глубину, хвостовая версия безопаснее по стеку.
- Потоки `Seq` с `filter`/`fold` позволяют обойтись без явных циклов.
- `map` удобно, чтобы пометить кандидаты и дальше фильтровать.
- Ленивые последовательности дают работу с бесконечными данными; императивный цикл оставлен для сравнения.
