...
- Решения задачи (модуль `Fp_first_lab.Euler7`):
  ...
  - `modular_pipeline`: последовательность `ints_from 2 |> Seq.filter is_prime |> Seq.drop (n-1) |> Seq.uncons`.
  - `mapped_generation`: генерация пар через `map`, последующая фильтрация по признаку простоты и выбор нужного элемента потока.
  - `loop_based`: императивный `while`-цикл для сравнения с традиционным стилем.
  - `lazy_seq_based`: работа с бесконечным ленивым списком простых (`primes_seq () |> Seq.drop (n-1) |> Seq.uncons`).
